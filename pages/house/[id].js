...
const House = ({ estates }) => {
  return (
    <div id="rent" className="w-full">
      ...
      <h2 className="text-xl md:text-2xl mt-2  uppercase p-2"> Available house for sale</h2>
          
        </div>

        <div className="grid grid-cols md:grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-8 mt-2">
          {estates.map((estate) => (
            <div
              className=" 
        ...
        <Link href={`/desc/${estate._id}`}>
          ...
          <button className="px-8 py-2 text-xl mt-4 mr-8 bg-[#eee3bc] text-black font-bold rounded mb-10">
              ...
              <div className="p-6  rounded-xl bg-[#eee3bc]">
                ...
                <p className="pb-2 flex justify-center text-black  text-bold text-2xl">  <ImPriceTag size={30} />  ₦{estate.price}</p>
                <p className="text-bold text-black pb-2 flex justify-center text-xl pt-2">
                <FaBed size={30} /> {estate.title}
                </p>
                <p className="pb-2 flex justify-center text-black text-xl"> <MdLocationOn size={30} /> {estate.location} </p>
                {/* <p className="pb-2 flex justify-center text-black text-2xl "> <FaBath size={30} /> {estate.slug}</p> */}
